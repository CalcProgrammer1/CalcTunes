...
*/
    ...
    if(!libhandle) {
        if(sdk > 8) libhandle = dlopen("/data/data/com.calcprogrammer1.calctunes/lib/libatrack9.so", RTLD_NOW);
        else libhandle = dlopen("/data/data/com.calcprogrammer1.calctunes/lib/libatrack8.so", RTLD_NOW);
	if(libhandle) {
